...
if($_POST)
{
	...
	if(!empty($g_secret_key) && (!empty($value['g-recaptcha-response'])))
	{
			...
			// API FUNCTION CALL //
				switch ($export_mail_type) 
				{
					case 'tz_aweber':
						...
						break;
					case 'tz_active':
						themezaa_activecampaign($subscribe_email,$user_name,$msg_subscribed_success,$msg_invalid_api);
						break;
					case 'tz_custom':
						themezaa_email($subscribe_email,$user_name,$phone_num=NULL,$comment=NULL,$purchase_email,$email_subject,$msg_php_email_sent,$msg_php_email_not_sent);
						break;
					case 'tz_campain':
					...
					case 'tz_response':
						...
						break;
					case 'tz_mailchimp':
						themezaa_mailchimp($subscribe_email,$user_name,$last_name=NULL,$msg_subscribed_success,$msg_invalid_api,$msg_mailchimp_error);
						break;
					case 'tz_mailerlite':
						themezaa_mailerlite($subscribe_email,$user_name,$msg_subscribed_success,$msg_invalid_api);
						break;
					default:
 	...
 	// API FUNCTIONS //
	function themezaa_email($subscribe_email,$user_name,$phone_num=NULL,$comment=NULL,$purchase_email,$email_subject,$msg_php_email_sent,$msg_php_email_not_sent)
	{
		$to = $purchase_email;
		$from_email = 'noreply@themezaa.com'; 
	...
	function themezaa_mailchimp($subscribe_email,$user_name,$last_name = NULL,$msg_subscribed_success,$msg_invalid_api,$msg_mailchimp_error)
	{
		if(defined('mailchimp_api_key')	&& defined('mailchimp_api_listid'))
		{
			...
			$result = json_decode(themezaa_mailchimp_add_subscriber($subscribe_email,$status,$mailchimp_api_listid,$mailchimp_api_key,$user_name,$last_name));
	...
	function themezaa_activecampaign($subscribe_email,$user_name,$msg_subscribed_success,$msg_invalid_api)
	{
	...
	function themezaa_mailerlite($subscribe_email,$user_name,$msg_subscribed_success,$msg_invalid_api)
	{
		if(defined('ml_api_key') && defined('ml_groupid'))
		{
			...
			if($result['email'] == $subscribe_email){
				$themezaa_result = json_encode(array('type'=>'tz_message', 'text' => $msg_subscribed_success));
				echo $themezaa_result;die;
			}else{
